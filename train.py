...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--unet", type=str, default="original",
    ...
    parser.add_argument("--allow_cpu", action="store_true",
                        help="allow CPU training when CUDA/MPS is unavailable")
...
def get_training_device(allow_cpu: bool) -> torch.device:
    ...
    if cuda_ok:
        ...
        print(f"[INFO] Training device: {device} ({cuda_info})")
        return device

    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        device = torch.device("mps")
        print(f"[INFO] Training device: {device}; CUDA unavailable: {cuda_info}")
        return device

    if allow_cpu:
        print(f"[WARN] Training on CPU; CUDA unavailable: {cuda_info}")
        return torch.device("cpu")

    raise RuntimeError(
        "CUDA is not healthy, refusing to train on CPU. "
        f"Reason: {cuda_info}. Pass --allow_cpu only for intentional CPU tests."
    )
