...
def parse_args():
    ...
    parser.add_argument('--no_mobileone_reparam', action="store_true",
                        help="keep MobileOne train-time graph instead of reparameterizing it for inference")
...
def load_model(
    ...
    unet: str,
    device: torch.device,
    mobileone_reparam: bool = True,
) -> torch.nn.Module:
    net = create_model(mode, unet).to(device)
    net.load_state_dict(load_checkpoint_state(checkpoint_path, device))
    net = maybe_reparameterize_mobileone(net, unet, enabled=mobileone_reparam)
    net.eval()
    return net
...
def run(args, device: Optional[torch.device] = None):
    ...
    net = load_model(
        ...
        args.unet,
        device,
        mobileone_reparam=not args.no_mobileone_reparam,
    )
