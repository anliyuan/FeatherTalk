...
def main():
    ...
    parser.add_argument("--no_mobileone_reparam", action="store_true",
                        help="export MobileOne train-time graph instead of the reparameterized graph")
    parser.add_argument("--opset", type=int, default=11)
    args = parser.parse_args()

    net = create_model(args.asr, args.unet).eval()
    net.load_state_dict(load_checkpoint_state(args.checkpoint, torch.device("cpu")))
    net = maybe_reparameterize_mobileone(
        net,
        args.unet,
        enabled=not args.no_mobileone_reparam,
    ).eval()
